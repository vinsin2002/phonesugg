...
llm = ChatGroq(temperature=0, groq_api_key="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", model_name="mixtral-8x7b-32768")
...
examples = [
    ...
    {"input": "show me blue colour phones ","query": "SELECT phonedetails.*, soc.* FROM phonedetails JOIN soc ON phonedetails.pid = soc.sid where phonedetails.pname like '%blue%' LIMIT 10;"},
    {"input": "Recommend phones with great cameras.","query": " SELECT phonedetails.*, soc.* FROM phonedetails JOIN soc ON phonedetails.pid = soc.sid WHERE phonedetails.price BETWEEN 7000 AND 19000 and phonedetails.RearCamera is not null and phonedetails.FrontCamera is not null ORDER BY phonedetails.RearCamera, phonedetails.FrontCamera DESC LIMIT 10;"},
